...
export async function createModel(request, page) {
  ...
  const filterStatus =
    getAppSearch(request, sessionKeys.appSearch.filterStatus) ?? []
  const sortField =
    getAppSearch(request, sessionKeys.appSearch.sort) ?? undefined
  ...
  if (apps.total > 0) {
    ...
    return {
      ...
      header: getApplicationTableHeader(
        getAppSearch(request, sessionKeys.appSearch.sort)
      ),
      ...pagingData,
      searchText,
