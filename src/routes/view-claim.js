...
export const getPigTestResultRows = (data) => {
  ...
  const pigTestResultRows = [
    buildKeyValueJson(
      testResultText,
      `${testResultType.toUpperCase()} ${testResult}`,
      true
    )
  ]
  ...
  if (geneticSequencing) {
    ...
    pigTestResultRows.push(
      buildKeyValueJson(
        'Genetic sequencing test results',
        geneticSequencingLabel,
        true
      )
    )
  }
...
export const viewClaimRoute = {
  ...
  options: {
    ...
    handler: async (request, h) => {
      ...
      const claimDate = buildKeyValueJson(
        'Claim date',
        formattedDateToUk(createdAt),
        true
      )
      ...
      const dateOfVisit = {
        ...buildKeyValueJson(
          'Date of visit',
          formattedDateToUk(data?.dateOfVisit),
          true
        ),
      ...
      const vetName = {
        ...buildKeyValueJson(
          "Vet's name",
          upperFirstLetter(data?.vetsName),
          true
        ),
      ...
      const piHunt = buildKeyValueJson(
        'PI hunt',
        upperFirstLetter(data?.piHunt),
        true
      )
      ...
      const diseaseStatus = buildKeyValueJson(
      ...
      const sheepEndemicsPackage = buildKeyValueJson(
        ...
        upperFirstLetter(sheepPackages[data?.sheepEndemicsPackage]),
        true
      )
