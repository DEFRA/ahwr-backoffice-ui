...
export const agreements = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        let filterStatus = getAppSearch(
          request,
          sessionKeys.appSearch.filterStatus
        )
    ...
    options: {
      ...
      handler: async (request, h) => {
        try {
          ...
          const { searchText, searchType } = searchValidation(
            request.payload.searchText
          )
          setAppSearch(
            request,
            sessionKeys.appSearch.searchText,
            searchText ?? ''
          )
          setAppSearch(
            request,
            sessionKeys.appSearch.searchType,
            searchType ?? ''
          )
