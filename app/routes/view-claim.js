  ...
  options: {
    ...
    handler: async (request, h) => {
      ...
      const status = {
        key: { text: "Status" },
        value: {
          html: `<span class='app-long-tag'><span class='govuk-tag responsive-text ${getStyleClassByStatus(claimStatus.replaceAll(/_/g, " "))}'> ${upperFirstLetter(claimStatus.replaceAll(/_/g, " ").toLowerCase())} </span></span>`,
        },
      ...
      return h.view("view-claim", {
        ...
        status: {
          normalType: upperFirstLetter(claim.status.replaceAll(/_/g, " ").toLowerCase()),
          tagClass: getStyleClassByStatus(claim.status.replaceAll(/_/g, " ")),
        },
